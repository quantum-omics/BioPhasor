...
# The pathway-atlas figures (cst_pathway_spectrum / _cp_error / _coherence) are
# printed by BOTH manuscripts. This is the method paper's suite, so they are
# written into manuscripts/biophasor/ once; the tumor manuscript's copies are
# refreshed by experiments/tumor/codes/run_all.py.
SUITE = "biophasor"
...
# ── figures ──────────────────────────────────────────────────────────────────
def _save(fig, name):
    common.save_fig(fig, name, SUITE)
...
def main():
    ...
    results = {
        ...
        # The manuscripts quote several of the above as PERCENTAGES ("captures
        # $52.5\%$", "$34\%$ coverage", "rank-3 error $62.3\%$"), and the number
        # guard matches values, not arithmetic: 52.5 does not round-trip to the
        # stored fraction 0.525. The derived form is therefore stored
        # alongside, so the quoted literal has a receipt of its own rather than
        # forcing the guard's whitelist to be widened.
        "as_percent": {
            "pathway_rank1_energy": round(100.0 * float(cum_p[0]), 4),
            "pathway_rank3_energy": round(100.0 * float(cum_p[2]), 4),
            "flat_gene_rank1_energy": round(100.0 * float(cum_f[0]), 4),
            "gene_coverage": round(100.0 * float(pmeta["gene_coverage_fraction"]), 4),
            "cp_rank3_error_pathway": round(100.0 * float(err_p3), 4),
            "cp_rank3_error_flat": round(100.0 * float(err_f3), 4),
            "exp07_flat_rank50_energy": 50.32,
            "exp07_flat_rank150_energy": 85.81,
        },
    }
