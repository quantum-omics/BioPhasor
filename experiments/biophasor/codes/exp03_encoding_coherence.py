...
def run():
    ...
    for name, fn in ENCODERS.items():
        ...
        enc_out[name] = {
            ...
            # The manuscript makes the non-discriminativeness argument in
            # percent ("retains 24,306/24,504 = 99.2\% of genes"), and the
            # number guard matches values rather than arithmetic, so the
            # percentage form needs a receipt of its own.
            "pct_genes_C_gt_0.30": round(100.0 * float((C > C_THRESHOLD).mean()), 2),
            "n_genes_total": int(C.size),
        }
