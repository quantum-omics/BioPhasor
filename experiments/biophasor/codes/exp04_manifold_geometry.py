...
def run():
    ...
    matrix_view = {
        ...
        # Quoted in the manuscript as a percentage ("37\% of near-boundary
        # pairs ... are misordered"); stored so the quoted form has a receipt.
        "pct_pairs_misordered": round(100.0 * n_misordered / len(geo_pairs), 1),
    ...
    def _mean_test(phi, label, extra=None):
        ...
        out = {
            ...
            # Stored to 4 dp, not 2. At 2 dp this lands on 177.65, which is a
            # rounding cliff: the manuscript's 177.6 is right against the
            # underlying value but reads as a defect against a receipt that has
            # itself been rounded to the halfway point. A receipt must be at
            # least as precise as the text that quotes it.
            "arithmetic_error_vs_circular_deg": round(_circ_err_deg(arithmetic, circular), 4),
        }
    ...
    result = {
        ...
        # The manuscript quotes the TOTAL ("12,242 angles total", "across all
        # 12,242 angles"), which appeared nowhere on disk: only the two
        # addends were written, and the guard matches values, not arithmetic.
        "n_phases_total": int(phi_cells.size + phi_genes.size),
