...
Discipline: seeded (SEED=0), single-panel PNGs at dpi>=300 saved to BOTH
experiments/figures/ and manuscript/, results JSON with a top-level verdict.
...
RESDIR = os.path.join(EXPDIR, "results")
FIGDIR = os.path.join(EXPDIR, "figures")
# manuscript dir is repo_root/manuscript ; repo root is EXPDIR/../..
REPO_ROOT = os.path.abspath(os.path.join(EXPDIR, "..", ".."))
MSDIR = os.path.join(REPO_ROOT, "manuscript")
for d in (RESDIR, FIGDIR, MSDIR):
    os.makedirs(d, exist_ok=True)


def _save_both(fig, basename):
    """Save a figure to BOTH experiments/figures/ and manuscript/ (by basename)."""
    fp = os.path.join(FIGDIR, basename)
    fig.savefig(fp, dpi=300, bbox_inches="tight", facecolor="white")
    shutil.copyfile(fp, os.path.join(MSDIR, basename))
    return fp
