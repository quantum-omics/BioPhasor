"""
biophasor.core.encoder — CANONICAL phasor encoding functions + OmicsPhasorEncoder.

This is the single source of truth for phase encoding across the whole
platform (biophasor, phnn, spectral). `biophasor.transform.encoder` re-exports
...
def tanh_phase_encode(
    ...
    **Canonical tanh-phase encoding** (Manuscript Eq. 15, Notebook 1.1 default).
    ...
    eps : float, optional
        Deprecated alias for `epsilon` (accepted for compatibility with the
        former spectral-omics `connectome.phasor.tanh_phase_encode` signature).
        If given, overrides `epsilon`.
